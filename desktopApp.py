...
web_app_path = os.path.join(ROOT_DIR, 'src', "webApp.py")
...
def track_active_window():
    ...
    with open(activity_file_path, mode="a" if file_exists else "w", newline="", encoding="utf-8") as file:
        ...
        if not file_exists:
            writer.writerow(["activeWindow", "activeProgram", "activeTab", "startTime", "endTime", "timeSpent", "mouseClicks", "keyPresses", "task", "goal"])
...
def show_dashboard():
    try:
        ...
        subprocess.Popen(["streamlit", "run", web_app_path], shell=True)
...
def task_manager_gui():
    ...
    active_window_data = pd.read_csv(activity_file_path)
